        </main>
        <footer class="b-footer app__footer">
          <div class="container">
            <div class="row">
              <div class="col-12 col-md-9 offset-md-2">

              </div>
            </div>
          </div>

          <a class="footer-badge" href="https://www.gafas.be" target="_blank">
            <div class="footer-badge__text">site by</div>
            <div class="footer-badge__logo">
              <img src=data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw== class="lazy" data-src="<?php echo get_template_directory_uri(); ?>/src/img/brand/logo-gafas.svg" alt="Gafas.">
            </div>
          </a>
        </footer>
      </div>
    <?php wp_footer(); ?>
  </body>
</html>
